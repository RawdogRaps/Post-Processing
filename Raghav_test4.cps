...
function onSection() {
  // --- START: CUSTOM INTER-OPERATION BLOCK ---
  if (!isFirstSection()) {
    // 1. Cleanup from the PREVIOUS operation
    writeln("");
    writeComment("--- END OF PREVIOUS OPERATION ---");
    writeBlock("M05");
    writeBlock("M09");
    ...
    // 2. Setup for the NEW (current) operation
    writeComment("--- START OF NEW OPERATION ---");
  ...
  // --- END: CUSTOM INTER-OPERATION BLOCK ---
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel || state.tcpIsActive) {
    // The lines below are commented out because our custom block above now handles them.
  ...
  // These following functions are still needed! They output operation-specific details
  // like the correct tool number, spindle speed, and coolant type that your
  // hardcoded block cannot know.
  ...
  // The line below is commented out because our custom block now handles all initial modal commands.
  ...
  if (isProbeOperation()) {
    ...
    writeBlock(settings.probing.macroCall, "P" + 9832);
  ...
  if (subprogramsAreSupported()) {
    subprogramDefine(initialPosition, abc);
  }
}
