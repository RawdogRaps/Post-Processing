...
function onOpen() {
  ...
  // --- INSERT YOUR DESIRED INITIAL PROGRAM STARTUP LINES HERE ---
  // These are the new lines for the beginning of EACH program
  writeBlock("G90 G94 G17 G49 G40 G80");
  writeBlock("G20");
  writeBlock("G28 G91 Z0.");   // Z-axis home in incremental mode
  writeBlock("G90");           // Absolute positioning
  writeBlock("G28 G91 X0. Y0.");// X and Y axis home in incremental mode
  writeBlock("G90");           // Absolute positioning (again, good practice to ensure)
  // --- END OF INSERTION ---
...
function onSection() {
  ...
  // Custom logic for intermediate cleanup BETWEEN operations
  if (!isFirstSection()) { // This condition is correct for inter-operation cleanup
    writeln(""); // Blank line before cleanup
    writeComment("--- END OF PREVIOUS OPERATION / INTERMEDIATE CLEANUP ---"); // Comment
    
    // Output your full desired cleanup sequence (M05, M09, G28 G91 Z0., G90, G49, G28 G91 X0. Y0., G90, M01)
    writeBlock("M05");
    writeBlock("M09");
    writeBlock("G28 G91 Z0.");
    writeBlock("G90");
    writeBlock("G49");
    writeBlock("G28 G91 X0. Y0.");
    writeBlock("G90");
    writeBlock("M01");
    
    writeln(""); // Blank line after cleanup

    // *** IMPORTANT: Now, add your highlighted "program start" block for the *next* operation. ***
    // This is the block that needs to start every program *after* the first one.
    // The previous `onSection` for the *old* operation just finished,
    // so this is the start of the *new* program/operation setup.
    writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(getProperty("useG95") ? 95 : 94), gPlaneModal.format(17), toolLengthCompOutput.format(49), gFormat.format(40), gFormat.format(80));
    writeBlock(gUnitModal.format(unit == MM ? 21 : 20)); // This outputs G20 or G21
    writeBlock("G28 G91 Z0.");
    writeBlock("G90");
    writeBlock("G28 G91 X0. Y0.");
    writeBlock("G90");
    writeln(""); // Add a blank line for spacing after this block

    // Force modals and any here to reset the post's state after our manual G-code injection
    forceModals();
    forceAny();
  }

  // --- The original onSection logic continues from here ---
  // This is where the post-processor usually handles initial rapid moves, tool changes,
  // spindle start, coolant on, and work offsets for the *current* operation.
  // We need to be careful not to duplicate things here that we just outputted.

  if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel || state.tcpIsActive) {
    // If you already outputted M05/M09 above, you can comment these out if they get duplicated.
    // Given your last output, it seems the M05/M09 are coming from your block, so these might be redundant.
    // if (insertToolCall && !isFirstSection()) {
    //   onCommand(COMMAND_STOP_SPINDLE);
    // }
    // writeRetract(Z); // This might be redundant if G28 G91 Z0. in your block is sufficient for safe Z.
    // Consider commenting this out and relying on your G28 for Z retract.

    if (isFirstSection() && machineConfiguration.isMultiAxisConfiguration()) {
      setWorkPlane(new Vector(0, 0, 0));
      forceABC();
    }
    forceXYZ(); // Keep this! Important for position resets.
  ...
  writeln(""); // Keep this for the operation comment space
  ...
  // These lines are critical for the start of the next toolpath's *specific* setup
...
function onClose() {
  ...
  // The 'writeRetract(Z)' line likely generates your 'G00 Z0.6'.
  // We want to add your custom lines immediately after that implicit action
  // but before the final M30/M02 and the '%' character.

  // The 'onCommand(COMMAND_STOP_SPINDLE)' already outputs M05.
  // The 'onCommand(COMMAND_COOLANT_OFF)' already outputs M09.
  // So you only need to add the homing and G90/G49/M01 lines.

  writeln("");
  writeBlock("G28 G91 Z0."); // Z-axis home in incremental mode
  writeBlock("G90");         // Absolute positioning
  writeBlock("G49");         // Tool length compensation cancel
  writeBlock("G28 G91 X0. Y0."); // X and Y axis home in incremental mode
  writeBlock("G90");         // Absolute positioning (again, good practice to ensure)
